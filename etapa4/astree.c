...
int astreeCheckNature(ASTREE * node){
    ...
    case ASTREE_UMIN:
        result = astreeCheckMathUnaryExpression(node);
        if (result == DATATYPE_INVALID){
            fprintf(stderr,"error at line %d : unary min operator expects word/byte types of operands.\n",node->lineNumber);
            return DATATYPE_INVALID;
        }
        return result;
...
int astreeCheckMathUnaryExpression(ASTREE * node){
